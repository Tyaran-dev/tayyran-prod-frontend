...
const FlightFilter: React.FC<FlightFilterProps> = ({
    ...
    return (
        <div className="w-full scrollbar-hide max-w-xs max-h-screen sticky top-4 overflow-y-auto p-4 mt-4 border rounded-lg shadow-md bg-white">
            ...
            {/* // Enhanced Departure Time Filter with Clear Option */}
            <div className="py-3">
                <div className="flex justify-between items-center">
                    <h4 className="text-base py-2 font-semibold">{t("departuretime.departuretimeheading")}</h4>
                    <div className="flex items-center gap-2">
                        {filterDepartureTime !== "any" && (
                            <button
                                onClick={() => onDepartureTimeChange("any")}
                ...
                {isExpanded.departureTime && (
                    <div className="grid grid-cols-2 gap-4 mt-2">
                        {departureTimeOptions.map((time) => (
                            <div
                                key={time.key}
                                className={`border-2 px-2 py-4 rounded-lg flex flex-col items-center gap-2 cursor-pointer hover:shadow transition ${filterDepartureTime === time.key
                                    ? 'bg-emerald-50 border-emerald-500 text-emerald-700'
                                    : 'border-gray-300 hover:border-gray-400'
                                    }`}
                                onClick={() => {
                                    // Toggle logic: if clicking the already selected filter, reset to "any"
                                    if (filterDepartureTime === time.key) {
                                        onDepartureTimeChange("any");
                                    } else {
                                        onDepartureTimeChange(time.key);
                                    }
                                }}
                            >
                                <DayFilter />
                                <span className="text-sm text-center">{time.label}</span>
                                {filterDepartureTime === time.key && (
